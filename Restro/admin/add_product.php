...
if (isset($_POST['addProduct'])) {
  ...
  if (empty($_POST["prod_code"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price'])|| empty($_POST['prod_quantity'])) {
    ...
    $postQuery = "INSERT INTO rpos_products (prod_id, prod_code, prod_name, prod_img, prod_desc, prod_price, quantity ) VALUES(?,?,?,?,?,?,?)";
    ...
    if ($postStmt) {
      $success = "Product Added" && header("refresh:1; url=add_product");
    } else {
